...
/// )
///
/// // With global search bar (golden standard topbar)
/// SigapAppBar(
///   title: 'Dashboard',
...
class SigapAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  /// Override the background color. Defaults to [SigapColorScheme.of(context).bgSurface].
  ...
  /// Optional search bar widget (e.g., [SigapSearchBar]).
  /// When provided, renders between the title and trailing/trailing actions.
  ...
  Size get preferredSize {
    // Note: actual height includes MediaQuery.padding.top at build time.
    // This is the minimum content height; Scaffold adds status bar padding.
    if (bottom != null) {
      return Size.fromHeight((bottomHeight ?? kToolbarHeight) + kToolbarHeight);
    }
  ...
  Widget build(BuildContext context) {
    final bgColor = backgroundColor ?? SigapColorScheme.of(context).bgSurface;
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            // Main row: back arrow + title column + search bar + trailing + actions
            SizedBox(
              ...
              child: Row(
                children: [
                  ...
                  // Search bar (between title and sync/trailing)
                  if (searchBar != null) ...[
                    searchBar!,
            ...
            if (bottom != null) ...[
              SizedBox(height: SigapSpacing.x11),
...
class SlaBadgeData {
  ...
  ///
  /// Colors:
  /// - Green (>2 days remaining): [SigapColorScheme.of(context).primary]
  /// - Amber (1-2 days remaining): [SigapColorScheme.of(context).warning]
  /// - Red (<1 day or overdue): [SigapColorScheme.of(context).danger]
  factory SlaBadgeData.fromDaysRemaining(BuildContext context, int? days) {
    final l10n = AppLocalizations.of(context)!;
    if (days == null) {
      return SlaBadgeData(
        backgroundColor: SigapColorScheme.of(context).textDisabled,
        textColor: SigapColorScheme.of(context).surface,
        label: 'N/A',
      );
    }
    ...
    if (days <= 0) {
      return SlaBadgeData(
        backgroundColor: SigapColorScheme.of(context).danger,
        textColor: SigapColorScheme.of(context).surface,
        label: l10n.overdue,
      );
    } else if (days < 1) {
